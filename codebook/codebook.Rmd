...
We include electoral alliances that take part in **at least two elections** (see section "[Electoral alliances]").
...
In Party Facts we collect party level information from external datasets. We do not include these external datasets but **extract party information only**. Therefore different datasets from the social sciences are linked but are not combined. Currently **`r pf_external$number_datasets` external datasets** are imported and around **`r pf_external$number_external_parties` external parties** are linked.
