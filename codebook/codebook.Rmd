...
```{r worldmap, echo=FALSE, message=FALSE, warning=FALSE}
## worldmap
...
Currently we include around **`r  pf_core$number_parties` core parties**.
...
around **`r  pf_external$number_external_parties` external parties**.
...
Currently, it includes **`r pf_core$number_countries` countries**. The
main focus of the project is on the national level but parties from
relevant sub-national units may be included as well, if they are covered
by several datasets.
...
For smaller parties, we may apply a staggered approach. All parties \>5%,
are linked during the initial import. Parties **\<5% and >1%**
...
year to identify the party. A short **description** should be included if
no share information is available (e.g. leader information: "PM Indra
Gandhi (1969-1977)" ).
...
- **Names** should be **capitalized** and cleaned with respect to spelling,
  characters etc.
- **Alternative names** are divided by a **slash**
  (e.g. [All-German Bloc / League of Expellees and Deprived of
...
- **Comment** – Comments about the coding to the party. May also include
  ...
  and alliances.
- **Data** – Additional data in [JSON](https://en.wikipedia.org/wiki/JSON)
  as a collection name–value pairs in an object. (e.g. '{"inclusion":
  "leader"}')
...
"alliance" category – see section "[Technical
...
information only**. Therefore different datasets from the social sciences
are linked but are not combined. Currently around **`r pf_external$number_datasets` external datasets** are imported and around
**`r pf_external$number_external_parties` external parties** are linked.
...
- The dataset should contain the following information as variables. (For
  detailed information see chapters "[Adding
