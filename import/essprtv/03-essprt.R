...
if(! exists("pf_party")) {  # time intense so avoiding rereading
  url <- "https://partyfacts.herokuapp.com/download"

  raw_pf_core <- read_csv(glue("{url}/core-parties-csv/"), na = "")
  raw_pf_ext <- read_csv(glue("{url}/external-parties-csv/"), na = "", guess_max = 50000)
  ...
  pf_party <- 
    raw_pf_core %>% 
    select(country, name_short, name, name_english, partyfacts_id, technical) %>% 
    inner_join(pf_ext)
}
...
prt <- 
  ...
  unite(ess_info, essround, ess_variable, ess_party, sep = " --- ", remove = FALSE) %>% 
...
# plot(pl)
ggsave("essprtv.png", pl, width = 20, height = 25, units = "cm")
