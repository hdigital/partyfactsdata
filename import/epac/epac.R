...
epac <- 
  ...
  mutate(
    ...
    party_name = if_else(party_id == 8711, "Latvijas Pirmā partija/Latvijas Ceļš", party_name),
    party_name = if_else(party_id == 3213, "Autonomie, Liberté, Participation, Écologie", party_name),
    party_name = if_else(party_id == 8712, "Latvijas Krievu savienība", party_name),
    party_name_en = if_else(party_id == 9105,  "New Democratic Power – FORCA", party_name_en),
    party_name_en = if_else(party_id == 9602, "Slovak Democratic and Christian Union – Democratic Party", party_name_en),
    party_accr = str_trim(party_accr),
    party_name = str_trim(party_name),
    party_name_en = str_trim(party_name_en)
  ) |> 
  ## select and rename variables
  select(countrycode, party_id, name_short = party_accr, name_english = party_name_en, year_first, year_last, share_year = elecyear, share, regonly) |>
